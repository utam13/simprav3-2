...
class Tender extends CI_Controller {
	...
	public function __construct()
    {
        ...
        $this->load->model('mod_tender');
    }

    public function rekapitulasi()
    {
        $data['halaman'] = "Tender - Rekapitulasi";
    ...
    public function rincian_paket($thn = "", $kelompok = "total", $page = 1, $limit = 20, $isicari = "")
    {
        ...
        $jumlah_data = $this->mod_tender->jumlah_rincian($data['thn'], $qcari, $qkelompok);
        ...
        $rincian = $this->mod_tender->rincian($limit_start, $limit, $data['thn'], $qcari, $qkelompok)->result();
        foreach ($rincian as $r) {
            ...
            $rup_penyedia = $this->mod_tender->cek_rup_penyedia($r->kd_rup_paket);
            if(!$rup_penyedia){
                $subrecord['kegiatan'] = "-";
            } else {
            ...
            $pegawai = $this->mod_tender->cek_pegawai($r->nip_ppk);
            ...
            // cek tahapan tender
            $tahapan = $this->mod_tender->tahapan($r->kd_tender);
            if(!empty($tahapan)){
                $subrecord['tahapan'] = $tahapan['tahapan'] == "" ? "-" : $tahapan['tahapan'];
            }else {
                $subrecord['tahapan'] = "-";
            }
            ...
            $penyedia = $this->mod_tender->cek_penyedia($r->kd_penyedia);
            if(!empty($penyedia)){
        ...
        $total_pagu = $this->mod_tender->total_pagu($data['thn'], $qkelompok);
        $total_tender = $this->mod_tender->total_tender($data['thn'], $qkelompok);
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('layout/menu');
        $this->load->view('page/tender_rincian_paket');
        $this->load->view('layout/footer');
    }
    ...
    public function tender($thn = 2021)
    {
        ...
        $metode = $this->mod_tender->metode($thn)->result();
        foreach ($metode as $m) {
        ...
        $total_paket_batal = 0;
        $total_total_pagu_anggaran = $this->mod_tender->total_pagu_anggaran($thn);
        $total_pagu_anggaran_selesai = 0;
        $total_harga_negosiasi = 0;
        $total_hemat_optimalisasi = 0;

        $jenis = $this->mod_tender->jenis($thn)->result();
        foreach ($jenis as $j) {
        ...
        $total_pagu_anggaran = $this->mod_tender->total_anggaran($thn);

        $satker = $this->mod_tender->satker($thn)->result();
        foreach ($satker as $s) {
            if($s->paket > 0){
                $subrecord_satker_baris['no'] = $no;

                $cek_satker = $this->mod_tender->cek_satker($s->kd_satker);
